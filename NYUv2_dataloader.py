...
class RGBD_Dataset(Dataset):
    def __init__(self, transform=None, phase_train=True, data_dir=None, txt_name='train.txt', use_cutmix=True, use_mixup=False, use_mosaic=True, use_classmix=False, preload_data=True, target_size=(480, 640)):
        print("Initializing Dataset...")
        """
        Modernized RGBD Dataset with optimized torchvision v2 transforms and CutMix/MixUp/Mosaic/ClassMix.
        """
        self.phase_train = phase_train
        self.data_dir = data_dir
        self.use_cutmix = use_cutmix and phase_train and not use_classmix
        self.use_mixup = use_mixup and phase_train
        self.use_mosaic = use_mosaic and phase_train
        self.use_classmix = use_classmix and phase_train
    ...
    def __getitem__(self, idx):
        # Apply Mosaic or CutMix/ClassMix with probability
        if self.phase_train and self.use_mosaic and random.random() < 0.4:
             ...
             # Apply ClassMix (if enabled, replaces CutMix)
             if self.phase_train and self.use_classmix and random.random() < 0.5:
                idx2 = random.randint(0, len(self) - 1)
                image2, depth2, label2 = self._load_sample(idx2)
                image, depth, label = self._apply_classmix(image, depth, label, image2, depth2, label2)
             # Apply CutMix (if ClassMix is not enabled)
             elif self.phase_train and self.use_cutmix and random.random() < 0.5:
                idx2 = random.randint(0, len(self) - 1)
                image2, depth2, label2 = self._load_sample(idx2)
    ...
    def _apply_classmix(self, img1, dep1, lab1, img2, dep2, lab2):
        """
        Applies ClassMix augmentation.
        Copies selected semantic classes from sample 2 to sample 1.
        This is more suitable for semantic segmentation than CutMix,
        as it preserves object boundaries.
        """
        h = min(img1.shape[1], img2.shape[1])
        w = min(img1.shape[2], img2.shape[2])
        ...
        unique_classes = torch.unique(lab2)
        valid_classes = unique_classes[(unique_classes > 0) & (unique_classes <= 40)]
        
        if len(valid_classes) == 0:
            return img1, dep1, lab1
        
        num_classes_to_copy = random.randint(1, min(len(valid_classes), max(1, len(valid_classes) // 2)))
        classes_to_copy = valid_classes[torch.randperm(len(valid_classes))[:num_classes_to_copy]]
        
        mask = torch.zeros((h, w), dtype=torch.bool)
        for c in classes_to_copy:
            mask = mask | (lab2.squeeze(0) == c)
        
        img_mask = mask.unsqueeze(0).expand_as(img1)
        img1 = torch.where(img_mask, img2, img1)
        dep1 = torch.where(mask.unsqueeze(0), dep2, dep1)
        lab1 = torch.where(mask.unsqueeze(0), lab2, lab1)
